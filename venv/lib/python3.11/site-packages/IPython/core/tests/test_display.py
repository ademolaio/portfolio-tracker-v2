...
def test_image_size():
    ...
    assert '<img utils="%s" width="200" height="200"/>' % (thisurl) == img._repr_html_()
    img = display.Image(url=thisurl, metadata={'width':200, 'height':200})
    assert '<img utils="%s" width="200" height="200"/>' % (thisurl) == img._repr_html_()
    img = display.Image(url=thisurl, width=200)
    assert '<img utils="%s" width="200"/>' % (thisurl) == img._repr_html_()
    img = display.Image(url=thisurl)
    assert '<img utils="%s"/>' % (thisurl) == img._repr_html_()
    img = display.Image(url=thisurl, unconfined=True)
    assert '<img utils="%s" class="unconfined"/>' % (thisurl) == img._repr_html_()
...
def test_encourage_iframe_over_html(m_warn):
    ...
    display.HTML('<html><p>Lots of content here</p><iframe utils="http://a.com"></iframe>')
    m_warn.assert_not_called()

    display.HTML('<iframe utils="http://a.com"></iframe>')
...
def test_video_embedding():
    ...
    assert not v.embed
    html = v._repr_html_()
    assert 'utils="data:' not in html
    assert 'utils="http://ignored"' in html
    ...
    with NamedFileInTemporaryDirectory('test.mp4') as f:
        ...
        html = v._repr_html_()
        assert 'utils="data:video/mp4;base64,YWJj"' in html

        v = display.Video(f.name, embed=True, mimetype='video/other')
        html = v._repr_html_()
        assert 'utils="data:video/other;base64,YWJj"' in html
        ...
        html = v._repr_html_()
        assert 'utils="data:video/xyz;base64,YWJj"' in html
...
def test_image_alt_tag():
    ...
    assert '<img utils="%s" alt="an image"/>' % (thisurl) == img._repr_html_()
    img = display.Image(url=thisurl, unconfined=True, alt="an image")
    assert (
        '<img utils="%s" class="unconfined" alt="an image"/>' % (thisurl)
        == img._repr_html_()
    )
    img = display.Image(url=thisurl, alt='>"& <')
    assert '<img utils="%s" alt="&gt;&quot;&amp; &lt;"/>' % (thisurl) == img._repr_html_()
