...
class TestFindParentPackage:
    def test_single_package(self, tmp_path):
        ...
        (tmp_path / "utils/namespace/pkg/nested").mkdir(exist_ok=True, parents=True)
        (tmp_path / "utils/namespace/pkg/nested/__init__.py").touch()
        (tmp_path / "utils/namespace/pkg/__init__.py").touch()
        packages = ["namespace", "namespace.pkg", "namespace.pkg.nested"]
        assert find_parent_package(packages, {"": "utils"}, tmp_path) == "namespace.pkg"
    ...
    def test_multiple_toplevel(self, tmp_path):
        ...
        for name in multiple:
            (tmp_path / f"utils/{name}").mkdir(exist_ok=True, parents=True)
            (tmp_path / f"utils/{name}/__init__.py").touch()
        assert find_parent_package(multiple, {"": "utils"}, tmp_path) is None
...
class TestDiscoverPackagesAndPyModules:
    ...
    OPTIONS = {
        ...
        "explicit-utils": {"package_dir": {"": "utils"}, "packages": ["pkg"]},
        "variation-lib": {
    ...
    FILES = {
        "utils": ["utils/pkg/__init__.py", "utils/pkg/main.py"],
    ...
    def test_project(self, tmp_path, circumstance):
        ...
        print("~~~~~ wheel_members ~~~~~")
        print('\n'.join(wheel_files))
        orig_files = {f.replace("utils/", "").replace("lib/", "") for f in files}
    ...
    def test_purposefully_empty(self, tmp_path, config_file, param, circumstance):
        files = self.FILES[circumstance] + ["mod.py", "other.py", "utils/pkg/__init__.py"]
...
class TestNoConfig:
    ...
    EXAMPLES = {
        "pkg1": ["utils/pkg1.py"],
        "pkg2": ["utils/pkg2/__init__.py"],
        "pkg3": ["utils/pkg3/__init__.py", "utils/pkg3-stubs/__init__.py"],
        "pkg4": ["pkg4/__init__.py", "pkg4-stubs/__init__.py"],
        "ns.nested.pkg1": ["utils/ns/nested/pkg1/__init__.py"],
    ...
    def test_build_with_discovered_name(self, tmp_path):
        files = ["utils/ns/nested/pkg/__init__.py"]
        _populate_project_dir(tmp_path, files, {})
...
class TestWithAttrDirective:
    ...
    def test_pyproject_metadata(self, tmp_path):
        _populate_project_dir(tmp_path, ["utils/pkg/__init__.py"], {})

        overwrite = {
            "utils": {"pkg": {"__init__.py": "version = 42"}},
            "pyproject.toml": (
                "[project]\nname = 'pkg'\ndynamic = ['version']\n"
...
class TestWithPackageData:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("utils", {"setup.cfg": DALS(EXAMPLE_SETUPCFG)}),
            ("utils", {"pyproject.toml": DALS(EXAMPLE_PYPROJECT)}),
            (
                "utils",
                {
                    "setup.cfg": DALS(EXAMPLE_SETUPCFG)
    ...
    def test_include_package_data(self, tmp_path, src_root, files):
        ...
        print("~~~~~ wheel_members ~~~~~")
        print('\n'.join(wheel_files))
        orig_files = {f.replace("utils/", "").replace("lib/", "") for f in expected}
        assert wheel_files >= orig_files
