...
class TestSdistTest:
    ...
    def test_exclude_dev_only_cache_folders(self, source_dir):
        included = {
            ...
            "utils/.nox/py/lib/python3.12/site-packages/bar-2.dist-info/COPYING.txt": "",
    ...
    _EXAMPLE_DIRECTIVES = {
        "setup.cfg - long_description and version": """
            [metadata]
            name = testing
            version = file: utils/VERSION.txt
    ...
    def test_add_files_referenced_by_config_directives(self, source_dir, config):
        ...
        (source_dir / 'utils/VERSION.txt').write_text("0.42", encoding="utf-8")
        ...
        assert (
            'utils/VERSION.txt' in cmd.filelist.files
            or 'utils\\VERSION.txt' in cmd.filelist.files
        )
