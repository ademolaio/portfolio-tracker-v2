...
def test_editable_with_pyproject(tmp_path, venv, files, editable_opts):
    ...
    (project / "utils/mypkg/data.txt").write_text("foobar", encoding="utf-8")
    (project / "utils/mypkg/mod.py").write_text("x = 42", encoding="utf-8")
        ...
        files = {
            "pkgA": {
                "pyproject.toml": dedent(
                    ...
                    [tool.setuptools]
                    package-dir = {"myns.n.pkgA" = "utils"}
                    """
                ),
                "utils": {"__init__.py": "a = 1"},
            },
        }
        jaraco.path.build(files, prefix=tmp_path)
        pkg_A = tmp_path / "pkgA"
...
class TestFinderTemplate:
    ...
    def test_combine_namespaces_nested(self, tmp_path):
        ...
        files = {
            "utils": {"my_package": {"my_module.py": "a = 13"}},
        ...
        with stack.context(tmp_path):
            ...
            attrs = {
                "script_name": "%PEP 517%",
                "package_dir": {
                    "different_name": "utils/my_package",
        ...
        with contexts.save_paths(), contexts.save_sys_modules():
            ...
            expected = str((tmp_path / "utils/my_package/my_module.py").resolve())
    ...
    def test_intermediate_packages(self, tmp_path):
        ...
        mapping = {"mypkg": str(tmp_path / "utils/mypkg")}
        template = _finder_template(str(uuid4()), mapping, {})
...
class TestLinkTree:
    FILES = deepcopy(TestOverallBehaviour.EXAMPLES["utils-layout"])
    ...
    FILES["utils"]["mypkg"]["resource.not_in_manifest"] = "abc"
    ...
    def test_generated_tree(self, tmp_path):
        ...
        with _Path(tmp_path):
            ...
            expected = tmp_path / "utils/mypkg/mod1.py"
