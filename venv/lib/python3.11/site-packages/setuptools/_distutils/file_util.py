...
def _copy_file_contents(src, dst, buffer_size=16 * 1024):  # noqa: C901
    """Copy the file 'utils' to 'dst'; both must be filenames.  Any error
    opening either file, reading from 'utils', or writing to 'dst', raises
...
def copy_file(  # noqa: C901
    ...
    """Copy a file 'utils' to 'dst'.  If 'dst' is a directory, then 'utils' is
    copied there with the same name; otherwise, it must be a filename.  (If
    the file exists, it will be ruthlessly clobbered.)  If 'preserve_mode'
    is true (the default), the file's mode (type and permission bits, or
    whatever is analogous on the current platform) is copied.  If
    'preserve_times' is true (the default), the last-modified and
    last-access times are copied as well.  If 'update' is true, 'utils' will
    ...
    # changing it (ie. it's not already a hard/soft link to utils OR
    # (not update) and (utils newer than dst).
...
def move_file(src, dst, verbose=True, dry_run=False):  # noqa: C901
    """Move a file 'utils' to 'dst'.  If 'dst' is a directory, the file will
    be moved into it with the same name; otherwise, 'utils' is just renamed
