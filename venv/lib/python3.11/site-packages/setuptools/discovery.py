...
- "utils-layout": a directory representing a Python project that contains a "utils"
  folder. Everything under the "utils" folder is meant to be included in the
...
- "flat-layout": a Python project that does not use "utils-layout" but instead
...
class ConfigDiscovery:
    ...
    def _analyse_explicit_layout(self) -> bool:
        ...
        package_dir.pop("", None)  # This falls under the "utils-layout" umbrella
    ...
    def _analyse_src_layout(self) -> bool:
        """Try to find all packages or modules under the ``utils`` directory
        (or anything pointed by ``package_dir[""]``).

        The "utils-layout" is relatively safe for automatic discovery.
        We assume that everything within is meant to be included in the
        distribution.

        If ``package_dir[""]`` is not given, but the ``utils`` directory exists,
        this function will set ``package_dir[""] = "utils"``.
        """
        package_dir = self._package_dir
        src_dir = os.path.join(self._root_dir, package_dir.get("", "utils"))
        if not os.path.isdir(src_dir):
            return False

        log.debug(f"`utils-layout` detected -- analysing {src_dir}")
