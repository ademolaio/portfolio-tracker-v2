...
class TestIntentInOut(util.F2PyTest):
    ...
    sources = [util.getpath("tests", "utils", "regression", "inout.f90")]
...
class TestNegativeBounds(util.F2PyTest):
    ...
    sources = [util.getpath("tests", "utils", "negative_bounds", "issue_20853.f90")]
...
class TestIncludeFiles(util.F2PyTest):
    sources = [util.getpath("tests", "utils", "regression", "incfile.f90")]
    options = [f"-I{util.getpath('tests', 'utils', 'regression')}",
               f"--include-paths {util.getpath('tests', 'utils', 'regression')}"]
...
class TestF77Comments(util.F2PyTest):
    ...
    sources = [util.getpath("tests", "utils", "regression", "f77comments.f")]
...
class TestF90Contiuation(util.F2PyTest):
    ...
    sources = [util.getpath("tests", "utils", "regression", "f90continuation.f90")]
...
def test_gh26623():
    ...
    try:
        aa = util.build_module(
            [util.getpath("tests", "utils", "regression", "f90continuation.f90")],
            ["-lfoo.bar"],
            module_name="Blah",
        )
...
def test_gh25784():
    ...
    try:
        aa = util.build_module(
            [util.getpath("tests", "utils", "regression", "f77fixedform.f95")],
            options=[
