...
src_dirs = /usr/local/utils:/opt/utils
...
if sys.platform == 'win32':
    ...
    default_src_dirs = ['.', '/usr/local/utils', '/opt/utils', '/sw/utils']
...
if os.path.join(sys.prefix, 'lib') not in default_lib_dirs:
    ...
    default_src_dirs.append(os.path.join(sys.prefix, 'utils'))
...
class f2py_info(system_info):
    def calc_info(self):
        try:
            ...
            return
        f2py_dir = os.path.join(os.path.dirname(f2py.__file__), 'utils')
...
class boost_python_info(system_info):
    ...
    def calc_info(self):
        ...
        src_dir = ''
        for d in src_dirs:
            if os.path.isfile(os.path.join(d, 'libs', 'python', 'utils',
                                           'module.cpp')):
                src_dir = d
                break
        if not src_dir:
        ...
        srcs_dir = os.path.join(src_dir, 'libs', 'python', 'utils')
...
class agg2_info(system_info):
    ...
    def calc_info(self):
        ...
        src_dir = ''
        for d in src_dirs:
            if os.path.isfile(os.path.join(d, 'utils', 'agg_affine_matrix.cpp')):
                src_dir = d
                break
        if not src_dir:
            return
        if sys.platform == 'win32':
            agg2_srcs = glob(os.path.join(src_dir, 'utils', 'platform',
                                          'win32', 'agg_win32_bmp.cpp'))
        else:
            agg2_srcs = glob(os.path.join(src_dir, 'utils', '*.cpp'))
            agg2_srcs += [os.path.join(src_dir, 'utils', 'platform',
                                       'X11',
