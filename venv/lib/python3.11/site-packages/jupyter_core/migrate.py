...
def migrate_dir(src: str, dst: str) -> bool:
    """Migrate a directory from utils to dst"""
    log = get_logger()
...
def migrate_file(src: str | Path, dst: str | Path, substitutions: Any = None) -> bool:
    """Migrate a single file from utils to dst
...
def migrate_static_custom(src: str, dst: str) -> bool:
    """Migrate non-empty custom.js,css from utils to dst

    utils, dst are 'custom' directories containing custom.{js,css}
    """
    log = get_logger()
    migrated = False
