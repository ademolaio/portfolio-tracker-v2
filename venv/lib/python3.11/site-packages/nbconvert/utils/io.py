...
def link(src, dst):
    """Hard links ``utils`` to ``dst``, returning 0 or errno.
...
def link_or_copy(src, dst):
    """Attempts to hardlink ``utils`` to ``dst``, copying if the link fails.
    ...
    if link_errno == errno.EEXIST:
        ...
        # linking, or 'utils' and 'dst' are on different filesystems.
        shutil.copy(src, dst)
